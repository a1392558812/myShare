<template>
  <div class="shop-header">
    ...
    <div class="shop-header-discounts" v-if="info.supports" @click="toggleSupportShow">
      <!-- 三级表达式式，当数据还没有从后台获取的时候，二级表达式取值为undefined，三级表达式取值就会报错
            例如当数据还没有从后台获取 二级表达式：info.supports = undefined,
                                      三级表达式：info.supports[0].name：报错
                                                                      -->
    ...
    <transition name="fade">
      <div class="shop-brief-modal" v-show="ifShopShow">
        <div class="brief-modal-content">
          ...
          <div class="mask-footer"  @click="toggleShopShow">
            <span class="iconfont iconicon-test11"></span>
          </div>
        </div>
    ...
    <transition name="fade" v-if="info.supports">
      <div class="activity-sheet"  v-show="supportShow">
        <div class="activity-sheet-content">
          ...
          <ul class="list">
            ...
            <span class="content-tag">
                <span class="mini-tag" :class="supportClasses[info.supports[index].type]"  v-if="info.supports">{{support.name}}</span>
              </span>
...
<script>
  import {mapState} from 'vuex'
    export default {
        data() { // 初始化数据
            return {
              supportClasses: ['activity-pink','activity-red','activity-orange'],
              ifShopShow: false,
              supportShow: false,
            }
        },
        ...
        methods: {  // 定义函数
          toggleShopShow () {
            this.ifShopShow = !this.ifShopShow
          },
          toggleSupportShow () {
