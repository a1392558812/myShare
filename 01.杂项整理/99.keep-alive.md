...
keep-alive通过Map 缓存组件的 vnode 以及会给每个组件做一个标识，这样在渲染的时候就不会再执行组件的初始化函数。
...
const KeepAliveImpl: ComponentOptions = {
	...
	            }
		       }
			...
			*/
			queuePostRenderEffect(() => { // 更新prop,异步执行onVnodeMounted 钩子函数
		...
		sharedContext.deactivate = (vnode: VNode) => {
			...
			queuePostRenderEffect(() => { //  异步执行onVnodeUnmounted钩子函数
				if (instance.da) {
		...
		return () => {
			...
			if (children.length > 1) { // 如果KeepAlive中子组件超过了一个，则会在生产环境抛出警告并返回子组件列表
				if (__DEV__) {
					warn(`KeepAlive should contain exactly one component child.`)
				}
				current = null
				return children
			} else if ( // 如果KeepAlive中子组件超过了一个，则会在生产环境抛出警告并返回子组件列表
				!isVNode(rawVNode) ||
			...
			*/
			...
			if (cachedVNode) {
				...
				if (vnode.transition) {// 如果组件使用了transition过渡动画则执行动画
...
当组件中传入了max属性且已缓存的组件数超过了max，就将最旧的缓存实例删除
