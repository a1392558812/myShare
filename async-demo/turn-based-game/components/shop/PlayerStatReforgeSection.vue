<template>
  <div class="stat-reforge-section">
    <h3>🔄 属性加点重铸</h3>
    <p class="section-desc">花费金币，将手动分配的属性点返还到未分配池，重新分配！初始点数和升级获得的点数不会被重铸。</p>
...
import { PLAYER_CONFIG } from "../../stores/constants.js";
...
const allocatedPointsList = computed(() => {
  ...
  return stats.map(s => {
    const currentPoints = player[`${s.key}Points`] || 0;
    const initialPoints = PLAYER_CONFIG.INITIAL_POINTS[s.key] || 0;
