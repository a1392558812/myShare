<template>
  <appContainer>
    <layoutCom style="width: 400px;" title="自定义多棱柱" type="panel"
      ...
      <control-item label="棱柱边宽 (sideWidth):" :labelValue="sideWidth">
        <inputCom type="range" :min="30" :max="300" :step="1" v-model="sideWidth" />
      </control-item>
      <control-item label="棱柱边高 (sideHeight):" :labelValue="sideHeight">
        <inputCom type="range" :min="30" :max="400" :step="1" v-model="sideHeight" />
      </control-item>
      <control-item label="棱柱缩放比例 (scale):" :labelValue="scale">
        <inputCom type="range" :min="0.3" :max="3" :step="0.1" v-model="scale" />
      </control-item>
      <control-item label="顶面透明度:" :labelValue="topOpacity">
        <inputCom type="range" :min="0" :max="1" :step="0.1" v-model="topOpacity" />
      </control-item>
      <control-item label="底面透明度:" :labelValue="bottomOpacity">
        <inputCom type="range" :min="0" :max="1" :step="0.1" v-model="bottomOpacity" />
      </control-item>
      ...
      <control-item v-for="(item, index) in sideStyle" :key="index" :label="`第${index + 1}棱柱透明度:`"
        :labelValue="item.opacity">
        <inputCom type="range" :min="0" :max="1" :step="0.1" v-model="item.opacity" />
      </control-item>
    </layoutCom>

    <layoutCom style="min-width: 800px;" title="预览" type="preview">
      <template #preview>
        <PrismComponent :n="n" :sideWidth="sideWidth" :sideHeight="sideHeight" :gap="gap" :sideStyle="sideStyle.map((item, index) => ({
        ...
        }" @cssStyle="getCssStyle" v-model:scale="scale" v-model:rotationX="rotationX" v-model:rotationY="rotationY" />
      </template>
...
const camelToHyphen = (str) => str.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();

const getCssStyle = (cssStyle) => {
  const htmlStr = ` <div class="prism-container">
    <div class="prism-wrapper">
      <div class="prism">
        <div class="prism-face top">上底</div>
        <div class="prism-face bottom">下底</div>`+ Array.from({ length: n.value || 0 }, (_, i) => `
        <div class="prism-face side${i}">${i}</div>`).join('') + `
      </div>
    </div>
  </div>\n`
  ...
  Object.keys(cssStyle).forEach(key1 => {
    cssMap[key1] = Object.keys(cssStyle[key1]).map(key => `     ${camelToHyphen(key)}: ${cssStyle[key1][key]};\n`).join('');
  })
