 ...
 * @param {Object} currentUnit 暗影法师位置和状态
 * @param {Number} currentUnit.x 暗影法师x坐标
 * @param {Number} currentUnit.y 暗影法师y坐标
 * @param {Number} currentUnit.size 暗影法师大小(px)
...
export const drawShadowSorcerer = (canvasRef, currentUnit) => {
  if (!canvasRef) return
  ...
  // 选择基础像素数据
  let basePixels = SORCERER_FACE_DOWN
  if (direction === 'up') basePixels = SORCERER_FACE_UP
  else if (direction === 'left') basePixels = SORCERER_FACE_LEFT
  else if (direction === 'right') basePixels = SORCERER_FACE_RIGHT
  ...
  const frames = isMoving ? SORCERER_WALK_FRAMES : SORCERER_IDLE_FRAMES
