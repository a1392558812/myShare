  ...
  <div>
    ...
    <ul>
      <li v-for="(text, index) in [
        ...
        '但实际上成果却很让人失望：',
        '1. 因为每一个unit单位像素都要走一次绘制，实际上每一帧canvas都要进行难以想象数量次数的绘制渲染，canvas的性能无法与WebGL‌相抗衡，属于是一开始技术栈就整错了，以至于到后面一步错步步错。',
        '2. 还有一个问题是时序问题，比如执行攻击指令需要有一个2s的动画帧，那么下一个指令的执行需要在这2s动画帧结束后再去执行。我是按1s 60帧做的，如果要实现我想的效果，那么我需要一个全局的时间帧，记录每一次指令的开始帧与结束帧，例如释放技能火球，这个法术动画2s，那么他的起始帧假设是1000，那么他的结束帧就是1000+2*60，我需要严格的控制当前帧动画数组的push与删除，单纯的加settimeout是无法做到的，因为settimeout是异步的，无法保证时序，也无法对齐帧数变化',
        '3. 所以这个demo基本上算是失败了，现在将一些demo中的小demo整理了一下，展示一下。',
      ]" :key="index">{{ text }}</li>
    </ul>
  </div>
</template>
