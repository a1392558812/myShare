...
const renderDamageNumber = (ctx, e, toScreen) => {
  ...
  ctx.shadowBlur = 4;
  ctx.fillText(e.text, pos.x, pos.y + offsetY);
};

const renderMeleeSlash = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
  ...
  ctx.lineWidth = 4;
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, e.radius, e.angle - Math.PI / 3, e.angle + Math.PI / 3);
  ctx.stroke();
...
const renderFreezeCircle = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
...
const renderMagicFireball = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
  ...
  if (progress < 0.18) {
    ...
    ctx.shadowBlur = 20;
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, r * (1.5 + progress * 2), 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, r * 2 * (1 + progress * 0.3) * pulse, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, r * 1.8 * pulse, 0, Math.PI * 2);
  ctx.fill();
  ...
  for (let i = 0; i < 5; i++) {
    const angle =
      Math.PI * 1.5 + (i - 2) * 0.35 + Math.sin(e.elapsed * 0.04 + i) * 0.18;
    ...
    ctx.shadowBlur = 3;
    ctx.beginPath();
    ctx.arc(sx, sy, 1.4 + Math.sin(e.elapsed * 0.07 + i) * 0.6, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.shadowBlur = 10;
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, r * (1.0 + progress * 3), 0, Math.PI * 2);
  ctx.stroke();
  ...
  ctx.shadowBlur = 6;
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, r * (1.0 + progress * 2), 0, Math.PI * 2);
  ctx.stroke();
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    ctx.beginPath();
    ctx.arc(px, py, ps * (1 - progress * 0.5), 0, Math.PI * 2);
    ctx.fill();
  }
...
const renderExplosion = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
  const fade = 1 - progress;
  ...
  if (e.variant === "sacrifice") {
    ...
    ctx.shadowBlur = 14;
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, r * 0.28, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.shadowBlur = 6;
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, r * (0.6 + progress * 0.4), 0, Math.PI * 2);
    ctx.stroke();
    ...
    for (let i = 0; i < 6; i++) {
      ...
      ctx.shadowBlur = 6;
      ctx.beginPath();
      ctx.arc(px, py, 2.2 * fade + 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
  } else {
...
const renderDashTrail = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
  const fade = 1 - progress;
  ctx.globalAlpha = fade * 0.35;
...
const renderHitBlocked = (ctx, e, toScreen) => {
  const pos = toScreen(e.x, e.y, ctx);
  const progress = e.elapsed / e.duration;
  const fade = 1 - progress;
  ...
  ctx.shadowBlur = 8;
  ctx.fillText("格挡", pos.x, pos.y - 40);
};
...
const renderBossDeathExplosion = (ctx, e, toScreen) => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    ctx.shadowBlur = 4;
    ctx.beginPath();
    ctx.arc(px, py, 3 * fade + 1, 0, Math.PI * 2);
    ctx.fill();
  }
...
const renderBossSpawnAnim = (ctx, e, toScreen) => {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctx.shadowBlur = 4;
    ctx.beginPath();
    ctx.arc(px, py, 3 * (1 - progress * 0.5) + 1, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.shadowBlur = 12;
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, e.radius * 0.5 * (1 - fade + 0.2), 0, Math.PI * 2);
  ctx.fill();
...
const renderPhaseTransition = (ctx, e, toScreen) => {
  ...
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, e.radius * (1 + progress * 0.5), 0, Math.PI * 2);
  ctx.stroke();
  ctx.globalAlpha = 0.3 * fade;
  ctx.fillStyle = e.color;
  ctx.beginPath();
  ctx.arc(pos.x, pos.y, e.radius * (1 + progress * 0.3), 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
};
...
const renderFirePillarWarn = (ctx, e, toScreen) => {
  ...
  if (progress > 0.75) {
    ...
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, e.radius * (0.5 + eruptFade), 0, Math.PI * 2);
    ctx.fill();
  }
...
const effectRenderers = {
  damageNumber: renderDamageNumber,
  meleeSlash: renderMeleeSlash,
  freezeCircle: renderFreezeCircle,
  magicFireball: renderMagicFireball,
  explosion: renderExplosion,
  venomWarn: renderVenomWarn,
  bossDeathExplosion: renderBossDeathExplosion,
  bossSlamWarn: renderBossSlamWarn,
  bossSlamHit: renderBossSlamHit,
  shadowWave: renderShadowWave,
  bossSpawnAnim: renderBossSpawnAnim,
  phaseTransition: renderPhaseTransition,
  firePillarWarn: renderFirePillarWarn,
  voidTeleportOut: renderVoidTeleport,
  voidTeleportIn: renderVoidTeleport,
  screenShake: () => {},
  dashTrail: renderDashTrail,
  hitBlocked: renderHitBlocked,
};

export const renderEffect = (ctx, e, toScreen) => {
  const renderFn = effectRenderers[e.type];
  if (renderFn) {
    ctx.save();
    renderFn(ctx, e, toScreen);
    ctx.restore();
  } else {
    console.warn(`No renderer defined for effect type: ${e.type}`);
  }
};