...
- 生成并维护项目 Rule 文件，定义代码规范和架构约束，明确设定输入输出规范
...
- 明确技术架构
- 对需求进行拆解和分析，生成项目架构md文档，把模块任务点切割为小块
- AI评估潜在的技术风险同时调整方案和改进需求
- 为每个模块生成详细的plan列表，明确TODO大纲

### AI 生成代码

- 根据plan列表，顺序生成代码
- 在生成代码同时，人工干预检查ai的代码质量，组件提取，业务逻辑的微调

### AI 验证代码

- ai自身进行代码 code review

### 测试 -> 上线 -> 知识库更新

- code review收集反馈并学习，同步更新知识库
- 根据反馈优化 AI 提示词

## 如何保障AI代码质量

- 制定ai需要遵守的Rules
- 项目上下文不要过长，防止ai过长时间用于分析上下文/漏东西/使用旧文档/使用旧规则 --> 【语义偏移】
- 明确正确的注释生成，保证代码可读性，使得人工也可以干预修改部分很拗口的逻辑代码实现
- 反复优化提示词/Rules/注意事项/知识库/组件库

## 语义偏移形成

初期规划和规则设定 --> ai规划好任务 --> ai开始执行任务 --> 执行过程中添加/修改/删除Rules、文档、知识库、组件库等 --> 历史聊天记录过长 --> ai在生成内容时，很多旧内容被多次重复参考提及 --> 强化ai把多次重复的旧内容当作主要依据而不是最新内容 --> ai对长上下文不同位置的信息利用和参照权重并不均匀 --> 中间一些内容和要求被忽略，ai生成的内容与任务需求不一致 --> 语义偏移形成
