...
## sessionStorage在不同Tab页不共享

// sessionStorage 的设计初衷就是为了隔离，而不是共享。同一个网站，你复制链接新开Tab/手动输网址开Tab，sessionStorage各自有独立，互不干扰。即使是在浏览器右键 → 复制标签页 / 重复标签页，则会把原 Tab 的 sessionStorage 拷贝一份给新 Tab，之后两边修改、新增还是互相独立且不再同步。

|    存储方式    |     是否跨 Tab 共享     | 是否跨域名 | 是否持久化  |
| :------------: | :---------------------: | :--------: | :---------: |
| sessionStorage |          隔离           |     否     | 关 Tab 就清 |
|  localStorage  |    同源所有 Tab 共享    |     否     | 手动删才清  |
|     Cookie     | 同源所有 Tab / 请求都带 |   可配置   |   可过期    |
