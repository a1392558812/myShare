...
<script setup lang="ts">
  ...
  // 起始索引=header高 scrollTop需要减去header高才是实际的item滚动位置
  ...
  const endIndex = computed(() => {
    ...
    // 动态高度模式：累加item高,.找到第一个超过scrollBottom的索引
    let accumulatedHeight = 0;
    for (let i = 0; i < itemHeights.value.length; i++) {
      accumulatedHeight += itemHeights.value[i];
  ...
  const scrollToIndex = (index: number) => {
    ...
    if (isFixedHeight.value) {
      ...
      // 动态高度模式：累加目标索引之前所有item的高度+header高
      let accumulatedHeight = 0;
