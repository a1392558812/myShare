...
> 水文章，此处抽取的是[【wot-ui】](https://wot-ui.cn/component/img-cropper.html)的 img-cropper 组件。做了一些简化和把一些工具方法都提到一个文件里，方便复制
...
<template>
  ...
  <view
    v-if="modelValue"
    class="custom-img-cropper"
    :style="customStyle"
    @touchmove="preventTouchMove"
  >
    <!-- 展示在用户面前的裁剪框 -->
    <view class="custom-img-cropper-wrapper">
      <!-- 画出裁剪框 -->
      <view class="custom-img-cropper-cut">
        <!-- 上方阴影块 -->
        <view
          class="custom-img-cropper-cut-top"
          :style="[IS_TOUCH_END ? {} : hightLightStyle, { height: `${cutTop}px` }]"
        />
        <view
          class="custom-img-cropper-cut-middle"
          :style="{ height: `${cutHeight}px` }"
        >
          <!-- 左侧阴影块 -->
          <view
            class="custom-img-cropper-cut-left"
            :style="[IS_TOUCH_END ? {} : hightLightStyle, { width: `${cutLeft}px`, height: `${cutHeight}px` }]"
          />
          <!-- 裁剪框 -->
          <view
            class="custom-img-cropper-cut-body"
            :style="{ width: `${cutWidth}px`, height: `${cutHeight}px` }"
          >
            <!-- 内部网格线 -->
            <view class="is-gridlines-x" />
            <view class="is-gridlines-y" />
            <!-- 裁剪窗体四个对角 -->
            <view class="is-left-top" />
            <view class="is-left-bottom" />
            <view class="is-right-top" />
            <view class="is-right-bottom" />
          </view>
          <!-- 右侧阴影块 -->
          <view
            class="custom-img-cropper-cut-right"
            :style="IS_TOUCH_END ? {} : hightLightStyle"
          />
        </view>

        <!-- 底部阴影块 -->
        <view
          class="custom-img-cropper-cut-bottom"
          :style="IS_TOUCH_END ? {} : hightLightStyle"
        />
      </view>
    ...
    <canvas
      :id="canvasId"
      :canvas-id="canvasId"
      class="custom-img-cropper-canvas"
      :disable-scroll="true"
    ...
    <!-- 下方按钮 -->
    <slot name="footer">
      <view class="custom-img-cropper-footer">
        <view class="custom-img-cropper-footer-button">
          <view @click="handleCancel"> {{ cancelButtonText }} </view>

          <view v-if="!disabledRotate" @click="handleRotate"> 旋转 </view>

          <view @click="handleConfirm"> {{ confirmButtonText }} </view>
        </view>
      </view>
    </slot>
  </view>
...
<script lang="ts" setup>
  ...
  export interface ImgCropperPropsType {
    modelValue?: boolean;
    cancelButtonText?: string;
    confirmButtonText?: string;
    disabledRotate?: boolean;
    fileType?: "jpg" | "png"; // https://developers.weixin.qq.com/miniprogram/dev/api/canvas/wx.canvasToTempFilePath.html#%E5%8F%82%E6%95%B0
    quality?: number; // 0-1
  ...
  const props = withDefaults(defineProps<ImgCropperPropsType>(), {
    modelValue: false,
    cancelButtonText: "取消",
  ...
  const canvasId = ref<string>(`cropper${getUuid()}`); // canvas 组件的唯一标识符
  ...
  // 图片中心轴点距左的距离
  const imgLeft = ref<number>(info.value.windowWidth / 2);
  const imgTop = ref<number>((info.value.windowHeight / 2) * TOP_PERCENT);
  ...
  const imageStyle = computed<AnyStyleType>(() => {
    const style = {
      width: picWidth.value ? `${picWidth.value}px` : "auto",
      height: picHeight.value ? `${picHeight.value}px` : "auto",
      transform: `translate(${imgLeft.value - picWidth.value / 2}px, ${
        imgTop.value - picHeight.value / 2
      }px) scale(${imgScale.value}) rotate(${imgAngle.value}deg)`,
   ...
   * 逆转是否使用动画
   */
  const revertIsAnimation = (animation: boolean | { value: boolean }) => {
  ...
  const computeImgSize = () => {
    ...
    if (!INIT_IMGHEIGHT && !INIT_IMGWIDTH) {
      ...
      tempPicWidth =
        (imgInfo.value!.width / imgInfo.value!.height) * tempPicHeight;
    } else if (
      (!INIT_IMGHEIGHT && INIT_IMGWIDTH) ||
      (INIT_IMGHEIGHT && INIT_IMGWIDTH)
    ) {
      tempPicWidth = Number(INIT_IMGWIDTH);
      tempPicHeight =
        (imgInfo.value!.height / imgInfo.value!.width) * tempPicWidth;
    }
   ...
   */
  const initImageSize = () => {
    if (INIT_IMGWIDTH && typeof INIT_IMGWIDTH === "number") {
      picWidth.value = INIT_IMGWIDTH;
    }
    if (INIT_IMGHEIGHT && typeof INIT_IMGHEIGHT === "number") {
   ...
   */
  const detectImgPosIsEdge = (scale?: number) => {
    ...
    currentImgTop =
      ...
      currentImgTop
        ...
        : cutTop.value +
          cutHeight.value -
          (currentPicHeight * currentScale) / 2;
   ...
   *  {图片区} 开始拖动
   */
  const handleImgTouchStart = (event: any) => {
    ...
    if (event.touches.length === 1) {
      ...
      fingerDistance.value = Math.sqrt(width ** 2 + height ** 2);
    }
  };
   ...
   *  {图片区} 拖动中
   */
  const handleImgTouchMove = (event: any) => {
    ...
    if (event.touches.length === 1) {
      ...
      const scale =
        imgScale.value * (hypotenuse / Number(fingerDistance.value));
  ...
  watch(
    ...
    (newValue) => {
      if (newValue) {
        ...
        const [widthRatio, heightRatio] = props.aspectRatio
          .split(":")
          .map(Number);
        ...
        cutTop.value =
          (info.value.windowHeight * TOP_PERCENT - tempCutHeight) / 2;
...
<script module="animation" lang="wxs">
    function setAnimation(newValue, oldValue, ownerInstance) {
      // 小程序不支持 es6
      if (newValue) {
        var id = ownerInstance.setTimeout(function() { // eslint-disable-line
...
<style lang="scss" scoped>
  .custom-img-cropper {
    ...
    height: 100vh;
    z-index: 1;
    .custom-img-cropper-wrapper {
      position: relative;
      background: rgba(0, 0, 0, 0.45);
      .custom-img-cropper-cut {
        z-index: 9;
        position: absolute;
        width: 100vw;
        height: 100vh;
        ...
        pointer-events: none;
        .custom-img-cropper-cut-top,
        .custom-img-cropper-cut-left,
        .custom-img-cropper-cut-right,
        .custom-img-cropper-cut-bottom {
        ...
        .custom-img-cropper-cut-middle {
          display: flex;
        }
        .custom-img-cropper-cut-body {
    ...
    .custom-img-cropper-footer {
      position: fixed;
      z-index: 10;
      bottom: 0;
      width: 100%;
      .custom-img-cropper-footer-button {
        margin: 0 20px;
        padding-top: 20rpx;
