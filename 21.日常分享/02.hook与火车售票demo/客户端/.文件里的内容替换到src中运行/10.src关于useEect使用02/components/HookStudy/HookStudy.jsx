...
const HookChild =(props) =>{
  ...
  */
  useEffect(()=>{
    ...
    return () =>{
      console.log('🍔🍔🍔🍔🍔useEffect的return回调执行')
...
/*_______________________________________________________________________________________________________*/
  ...
  /*
  传递给 useEffect 的函数在每次渲染中都会有所不同，这是刻意为之的。事实上这正是我们可以在
  effect 中获取最新的 count 的值，而不用担心其过期的原因。每次我们重新渲染，都会生成新的 effect，
  替换掉之前的。
  */
  useEffect(() =>{
    document.querySelector('#thisP').addEventListener('click',PClick,false)
    return () =>{
      document.querySelector('#thisP').removeEventListener('click',PClick,false)
    }
  })
